...
# %Z 추가되어야 함
formats = ['%Y-%m-%d %H:%M', '%m/%d/%Y %H:%M', '%m/%d/%y %H:%M']
for deadline in ['deadline', 'schedule', '납품일정', '납품 일정', '납기일자', '납기 일자', '스케줄']:
    for line in mail['message'].split('\n'):
        if deadline in line:
            date_part = re.sub(r"[^A-Za-z0-9가-힣/: ]", '', line).split(deadline)[1]
            mail['deadline'] = re.search(r"\d.+", date_part).group()

            # 삼중포문.. 정말 하고싶지 않았는데요... 더 나은 방법 찾습니다..
            for format in formats:
                try:
                    mail['deadline'] = datetime.datetime.strptime(mail['deadline'], format)
                    break
                except ValueError:
                    # 나중엔 로그로 남길 것
                    print("ValueError :", mail['deadline'])
